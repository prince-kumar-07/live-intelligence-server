...
async function fetchNews({ country, category = "military", timespan = "24h", limit = 20 }, retries = 3) {
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      // GDELT returns plain string on rate limit instead of JSON
      if (typeof data === "string" && data.includes("limit")) {
        if (attempt < retries) {
          console.log(`  Rate limited — waiting 6s (attempt ${attempt}/${retries})...`);
          await sleep(6000);
          continue;
        }
        throw new Error("GDELT rate limit — wait 5 seconds between calls");
      }
      ...
      if (err.response?.status === 429 && attempt < retries) {
        console.log(`  Rate limited (429) — waiting 6s (attempt ${attempt}/${retries})...`);
        await sleep(6000);
        continue;
      }
      throw err;
...
(async () => {
  const news = await fetchNews({ country: "India" });
  console.log(news);
})();
